...
% Sparsity of mean vectors
s = 5;

% Thresholding parameter
lambda = sqrt(2*log(d)/n1);
...
% Generate 2 s-sparse vectors
r_perm1 = randperm(d);
r_perm2 = randperm(d);
mu1_aux = [randn(s, 1); zeros(d-s, 1)];
mu2_aux = [randn(s, 1); zeros(d-s, 1)];

mu1      = mu1_aux(r_perm1);
mu2_orig = mu2_aux(r_perm2);   % to be modified according to mean_shift_i
...
for i = 1 : length_exp_exp
    ...
    mu2 = mu1 + mean_shift_i*(mu2_orig-mu1)/norm(mu2_orig-mu1);

    % Generate samples
    X1 = mu1*ones(1,n1) + randn(d, n1);
    X2 = mu2*ones(1,n2) + randn(d, n2);

    % Estimate sample means
    mu1_est_nt = X1*ones(n1,1)./n1;
    mu2_est_nt = X2*ones(n2,1)./n2;

    % With thresholding
    mu1_est = mu1_est_nt;
    mu1_est(abs(mu1_est) <= lambda) = 0;

    mu2_est = mu2_est_nt;
    mu2_est(abs(mu2_est) <= lambda) = 0;
